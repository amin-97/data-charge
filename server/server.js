...
server.get("/latest-blogs", (req, res) => {
  ...
  Blog.find({ draft: false })
    .populate(
      "author",
      "personal_info.profile_img personaol_info.username personal_info.fullname -_id"
    )
    .sort({ publishedAt: -1 })
...
server.post("/create-blog", verifyJWT, (req, res) => {
  ...
  if (!title.length) {
    return res.status(403).json({ error: "You must add a title" });
  }
  ...
  if (!draft) {
    ...
    if (!content.blocks.length) {
      return res
        .status(403)
        .json({ error: "There must be some content to publish it" });
    }

    if (!tags.length || tags.length > 10) {
      return res.status(403).json({
        error: "Provide tags in order to publish this blog, Maximum 10",
      });
    }
  }
